...
class DFT_temporal_aliasing():
    ...
    def __init__(self):
        ...
        self.choose_N = widgets.IntSlider(value = self.N, min = 5, max = 50, step = 1, description = 'Periodization constant (N)', 
    ...
    def __init__figure(self):
        ...
        with self.out:
            ...
            self.axs[0].xaxis.set_major_locator(locator)
    ...
    def plot_DFT(self):
        ...
        locator = matplotlib.ticker.MultipleLocator(2)
        self.axs[2].xaxis.set_major_locator(locator)
        vector = np.vectorize(int)
        x_fourier = np.linspace(-10, 10, num = 21, endpoint=True)
        y_fourier = np.fft.fftshift(np.fft.fft(self.y_period_cut_dft))
